<template>
    ...
    }" :height="'100vh'" :opacity="0.7" :overlayColor="'25, 25, 25'">
        <div class="blur">
            <SkewBox :height="windowHeight * 0.7" :padding="'80px 0 0 0'" :divisionPosition="35"
                :leftColor="'linear-gradient(to bottom, var(--FSCred), rgba(var(--FSCredRGB), 0.5) 60%, transparent)'" :rightColor="'transparent'">
                <template v-slot:left>
                    ...
                    <div class="w-100">
                        <center>
                            ...
                            <h6 style="color: lightgray" v-html="professor.title"></h6>
                        </center>
                    </div>
                    <hr class="white-hr" />
            ...
            <div class="professor mx-5 py-5">
                <div class="container white">
                    <tabs :key="forceReload" :options="{ useUrlFragment: false, defaultTabHash: 'bio' }" nav-class="m-0 px-0 row tab-headers"
                        nav-item-class="col p-0" nav-item-link-class="tab-header vertical-center white no-decor" panels-wrapper-class="tab-panels p-3">
                        <tab v-if="professor.bio" id="bio" name="Biography" v-html=professor.bio></tab>
                        <tab v-if="professor.interests" name="Interests" v-html=professor.interests></tab>
                        <tab v-if="professor.education" name="Education" v-html=professor.education></tab>
                        <tab v-if="professor.awards" name="Honors/Awards" v-html=professor.awards></tab>
                        <tab v-if="professor.publications" name="Publications" v-html=professor.publications></tab>
                    </tabs>
                </div>
            </div>
